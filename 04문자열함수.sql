...
SELECT FISRT_NAME, LENGTH(FIRST_NAME), INSTR(FIRST_NAME, 'e') FROM EMPLOYEES;

--SUBSTR() -문자열자르기, CONCAT() -문자열 합치기
SELECT FIRST_NAME, SUBSTR(FIRST_NAME, 1, 3) FROM EMPLOYEES; --1번째에서 3글자를 나오게 자름
SELECT FIRST_NAME, CONCAT(FIRST_NAME, LAST_NAME), FIRST_NAME || LAST_NAME FROM EMPLOYEES;

--LPAD() -왼쪽채우기, RPAD() - 오른쪽 채우기
...
SELECT LTRIM(FIRST_NAME, 'A') FROM EMPLOYEES; --왼쪽에 처음발견되는 문자 A 제거
...
SELECT REPLACE ('HELLO WORLD  ~! ', ' ', '') FROM DUAL; --모든공백 제거
--중첩
SELECT REPLACE ('HELLO WORLD ~!','HELLO','BYE') FROM DUAL;
...
SELECT CONCAT('(02)', SUBSTR(PHONE_NUMBER, INSTR(PHONE_NUMBER,'.'),LENGTH(PHONE_NUMBER)))AS RESULT
FROM EMPLOYEES ;
