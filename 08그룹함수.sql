...
--그룹함수 : 그룹함수와 일반칼럼을 동시에 출력할 수 없습니다. (오라클만)
...
SELECT COUNT(COMMISSION_PCT), 
       DEPARTMENT_ID, 
       TRUNC(AVG(SALARY+COMMISSION_PCT),2) AS 월급의평균, 
       SUM(SALARY), 
       COUNT(*) 
FROM EMPLOYEES
GROUP BY DEPARTMENT_ID;
...
SELECT DEPARTMENT_ID, JOB_ID,
       --DECODE(GROUPING(DEPARTMENT_ID), 1, '토탈',DEPARTMENT_ID), 
       DECODE(GROUPING(JOB_ID), 1, '합계',JOB_ID) ,
       SUM(SALARY),
       COUNT(*)
FROM EMPLOYEES
GROUP BY ROLLUP(DEPARTMENT_ID, JOB_ID)
