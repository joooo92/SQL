...
INSERT INTO DEPTS VALUES(DEPTS_SEQ.NEXTVAL , 'TEST', 100, 1000); --X10( 시퀀스의 최대값에 도달하면 더이상 사용할 수 없음)
...
--2. 증가값을 음수로 변경
ALTER SEQUENCE DEPTS_SEQ INCREMENT BY -39; --현재 시퀀스를 -1로 감소
--3. 시퀀스 실행
...
--많이 변경되지 않는 일반 컬럼에 인덱스를 적용할 수 잇습니다.
...
--인덱스 생성(인덱스는 조회를 바르게 하긴 하지만, 무작위하게 많이 생성하면, 오히려 성능이 떨어질 수 있습니다. 
CREATE UNIQUE INDEX EMPS_IT_IDX ON EMPS_IT (FIRST_NAME);--유니크인덱스(컬럼값이 유니크해야합니다)

--인덱스 삭제
DROP INDEX EMPS_IT_IDX;

--인덱스는 (결함인덱스) 여러컬럼을 지정할 수 있습니다. 
...
--FIRST_NAME 기준으로 순서
-- 힌트주석 : /*+   */ 해당힌트주석을 받으면 RN값이 DESC순으로 출력됨
-- 인덱스를 기준으로 힌트를 주는 방법
