...
-- 주의할 점 , 단일행이어야합니다. 칼럼값도 1개이어야합니다.
SELECT * 
FROM EMPLOYEES
WHERE JOB_ID = (SELECT JOB_ID FROM EMPLOYEES WHERE EMPLOYEE_ID = 103 OR EMPLOYEE_ID = 104);
-- ^오류남. 이유: first_name = David가 여러 행이 존재하기 때문에 단일행 서브쿼리가 실행 되지 않는다
...
--직업이 IT_FROG인 사람들의 최소값보다 큰 급여를 받는 사람들
SELECT *
FROM EMPLOYEES
...
--ROWNUM은 조회된 순서이기떄문에, ORDER BY 과 같이 사용할경우 ROWNUM이 섞여나옴
SELECT FIRST_NAME,
       SALARY,
       ROWNUM
FROM(SELECT *
...
-- ROWNUM은 조회데이터라 무조건 1번째부터 조회가 가능하다. 중간부분에 대해서는 조회가 불가능
SELECT FIRST_NAME,
       SALARY,
       ROWNUM
FROM(SELECT *
...
--스칼라쿼리 - LEFT JOIN 과 같은 역할, 한번에 1개의 컬럼을 가져올때 
